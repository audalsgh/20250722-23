# 22~23일차

## 로보플로우에서 라벨링 -> 코랩, 런파드와 연결하여 모델 훈련하기
[교수님의 로보플로우-런파드 연동 자료](https://docs.google.com/document/d/14UZTOXzo2wvKDHm8MRjCrucP1JV6uojaU2NxWFSNO-A/edit?tab=t.0)<br>
[교수님의 로보플로우 자료](https://docs.google.com/document/d/1dX04LQyjvr5-s3gsnk4QqRllBLTgHZzXCGvCRgGFu7s/edit?tab=t.0)

1. <img width="1499" height="913" alt="image" src="https://github.com/user-attachments/assets/697b4e19-9175-4d19-ae3f-5dca045f2633" />
어노테이팅을 500장 완료했다고 가정.

2. <img width="841" height="914" alt="image" src="https://github.com/user-attachments/assets/193051be-1624-4bcf-824b-ed3b85cd17fd" />
모델 만들기에 들어가서 옵션 건드리지않고 버전 v1 생성

3. <img width="1461" height="932" alt="image" src="https://github.com/user-attachments/assets/c1e57a73-ca65-459a-b6ef-5b9e03f0d41b" />
다운로드 데이터셋 누르기

4. <img width="791" height="409" alt="image" src="https://github.com/user-attachments/assets/5da29c8f-5bcc-4f7b-82e4-9d51e029c4fb" />
yoloV8선택 후 show download code

5. <img width="795" height="434" alt="image" src="https://github.com/user-attachments/assets/472c1be6-8dac-49c4-bce0-30ced069c97c" />
코랩으로 넘어가서, api가 가려진 .snippet코드 전체를 가져오면된다.
